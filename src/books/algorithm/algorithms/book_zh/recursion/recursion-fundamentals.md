...
```typescript
/**
 * 递归反转链表 - 体现"假设子问题已解决"的思维模式
 * 
 * 【递归思维的关键】
 * 不要追踪递归展开的每一步！
 * 只需要问自己：如果后面的链表已经反转好了，我该如何处理当前节点？
 * 
 * 【递归关系】
 * 假设 head.next 及其后面的节点已经反转完成
 * 现在只需要把 head 接到反转后链表的末尾
 * 
 * @param head - 当前链表的头节点
 * @returns 反转后链表的新头节点
 */
function reverseList(head: ListNode | null): ListNode | null {
  ...
  // ========================================
  // 这里是"递归信仰跳跃"——
  // 我们相信 reverseList(head.next) 会正确返回反转后的新头节点
  // 不需要追踪它是如何工作的
  const newHead = reverseList(head.next);
  
  // ========================================
  // 合并步骤：把当前节点接到反转后链表的末尾
  // ========================================
  // 关键洞察：head.next 在反转后变成了链表的尾节点！
  // 
  // 反转前：head → (head.next) → ... → tail
  // 反转后：head → (head.next) ← ... ← tail
  //                    ↑
  //              现在是尾节点
  // 
  // 所以 head.next.next = head 就是让尾节点指向 head
  head.next.next = head;
  
  // 断开 head 原来指向 head.next 的链接
  // 否则会形成环！
  head.next = null;
 ...
 * 【为什么记忆化有效？】
 * 因为递归中存在大量"重叠子问题"
 * fib(5) 需要 fib(3)，fib(4) 也需要 fib(3)
 * 如果不缓存，fib(3) 会被计算多次
 * 
 * 时间复杂度：O(n) - 每个 fib(i) 只计算一次
 * 空间复杂度：O(n) - 存储 n 个结果 + 递归栈深度 O(n)
 */
