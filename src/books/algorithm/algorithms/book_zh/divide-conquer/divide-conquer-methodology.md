 ...
 * 
 * 【分治的本质】
 * "分而治之"——把复杂问题分解成更简单的子问题
 * 子问题与原问题结构相同，只是规模更小
 * 
 * 【三步曲】
 * 1. Divide（分解）：把问题分成若干子问题
 * 2. Conquer（解决）：递归地解决子问题
 * 3. Combine（合并）：把子问题的解合并成原问题的解
 * 
 * 【为什么分治高效？】
 * 关键在于"合并"步骤的效率
 * 如果合并的代价是 O(n)，分两半，递归深度是 O(log n)
 * 总复杂度：O(n log n)
 */
