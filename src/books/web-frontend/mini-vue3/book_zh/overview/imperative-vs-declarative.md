...
**代码即文档**：<code v-pre><div>{{ user.name }}</div></code> 一眼就能看出这里显示用户名。而命令式代码需要追踪 `innerText` 赋值才能确定 DOM 显示什么。
...
**哪些结构是稳定的**：`v-if` 和 `v-for` 会改变结构，但 <code v-pre><div>{{ text }}</div></code> 的结构是稳定的。编译器可以把结构稳定的区域标记为 Block，跳过子树遍历。
