...
**`Parser`（语法分析）**
- 核心职责：驱动整个解析流程，负责实现语法规则，构建 AST
- 它是整个解析器的"大脑"，定义了各种语法结构应该如何被解析

**`Tokenizer`（词法分析）**
- 核心职责：负责读取源代码字符流，并将其转换为 Token 序列
- 它是解析器的"眼睛"，负责识别代码中的每一个有意义的单元

**`State`（状态管理）**
- 核心职责：存储和管理解析过程中的所有上下文信息
- 它是解析器的"记忆"，追踪当前位置、行列号、语法上下文等
