...
app.use(async (req, res, next) => {
  ...
  // 静态文件处理 (例如 CSS, 图片等)
  if (url.match(/\.(css|jpg|png|svg)$/)) {
    const filePath = path.join(projectRoot, url);
    try {
      const fileContent = await fs.readFile(filePath);
      // 根据文件类型设置正确的 Content-Type (此处简化)
      res.end(fileContent);
    } catch (e) {
      // 文件不存在，交给下一个中间件
      next();
...
app.use(async (req, res, next) => {
  ...
  if (url.endsWith('.js') || url.endsWith('.ts')) {
    ...
    try {
      ...
      // 简单的裸模块路径重写
      sourceCode = sourceCode.replace(/from\s+['|"]([^\.'"/][^\'"]*)['|"]/g, (match, moduleName) => {
        // 这是一个非常简化的实现，真实 Vite 会进行更复杂的路径解析
  ...
  // 如果请求的是 node_modules 里的文件，也需要处理
