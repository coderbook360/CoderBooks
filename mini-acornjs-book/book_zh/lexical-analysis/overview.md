...
欢迎来到 mini-acornjs 的第二部分！在第一部分中，我们成功构建了一个强大的表达式解析器，它能够将 Token 序列转化为结构化的抽象语法树（AST）。但是，我们当时“理所当然”地认为 Token 已经存在了。现在，是时候揭开这个“黑盒”的神秘面纱了。

这一部分，我们将深入探索编译过程的第一个关键阶段——**词法分析（Lexical Analysis）**。

## 1. 词法分析：代码的“翻译官”

想象一下，当你阅读一篇文章时，你的大脑并不会把句子看作一长串没有分隔的字母。相反，你会自然地识别出单词、标点和段落。词法分析对于计算机来说，做的就是类似的工作。

对于计算机而言，你写的代码，比如 `let a = 1;`，最初只是一段平平无奇的字符串。它无法直接理解这段字符串的含义。词法分析器的角色，就是扮演一个“翻译官”，将这段对机器无意义的字符流，转换成一个个有明确含义的、结构化的单元。这个单元，我们称之为 **Token**。

这个过程通常也被称为 **分词（Tokenization）** 或 **扫描（Scanning）**，执行这个任务的程序则被称为 **词法分析器（Lexical Analyzer）**、**Tokenizer** 或 **Scanner**。
